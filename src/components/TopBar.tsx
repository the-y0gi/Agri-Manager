...
//     toggleLanguage(); // State update call
    
//     if (lang === "hi") {
//       toast.success("Language changed to English");
//     } else {
//       toast.success("भाषा हिंदी में बदल दी गई");
//     }
//   };
...
//           {/* --- Language Toggle Button (Updated onClick) --- */}
//           <button
...
import { useRouter } from "next/navigation";
import Link from "next/link";
import { Search, Settings, LogOut, X, Languages, Truck } from "lucide-react";
import toast from "react-hot-toast";
import { useLanguage } from "@/context/LanguageContext"; 
import { useLoader } from "@/context/LoaderContext";
...
const content = {
  en: {
    ...
    success: "Logged out successfully",
    fail: "Logout failed",
    drivers: "Drivers", // Added for tooltip/aria
  },
  hi: {
    welcome: "स्वागत है",
    searchPlaceholder: "किसान का नाम खोजें...",
    ...
    success: "सफलतापूर्वक लॉग आउट किया गया",
    fail: "लॉग आउट विफल रहा",
    drivers: "ड्राइवर", // Added for tooltip/aria
  },
};
    ...
    <header className="px-6 pt-14 pb-4 bg-white/80 backdrop-blur-md sticky top-0 z-40 border-b border-gray-100/50">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {/* --- DRIVER BUTTON (Added as per your requirement) --- */}
          <Link href="/drivers" onClick={showLoader}>
            <button
              className="p-2 rounded-full bg-gray-100 text-gray-600 hover:bg-emerald-50 hover:text-emerald-600 transition"
      ...
      {setSearchQuery && (
        <div className="relative group">
          ...
          <input
            type="text"
            placeholder={t.searchPlaceholder}
            className="w-full pl-12 pr-12 p-3.5 rounded-2xl bg-gray-100/80 border-transparent focus:bg-white focus:border-emerald-500/20 focus:ring-4 focus:ring-emerald-500/10 transition-all outline-none font-medium text-gray-700 placeholder:text-gray-400"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
          {searchQuery && (
