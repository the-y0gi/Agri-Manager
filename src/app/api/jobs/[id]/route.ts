...
// PUT: Update job (Work logs, Payments, Status, Job Details)
export async function PUT(request: NextRequest, { params }: Params) {
  try {
    ...
    if (body.updateJobDetails) {
      const { farmerName, mobileNumber } = body.updateJobDetails;
      if (farmerName) job.farmerName = farmerName;
      if (mobileNumber) job.mobileNumber = mobileNumber;
    }
