...
export default function JobDetailsPage() {
  ...
  const [editJobData, setEditJobData] = useState({ farmerName: "", mobileNumber: "" });
  ...
  const openEditJobModal = () => {
    if(job) {
        setEditJobData({ farmerName: job.farmerName, mobileNumber: job.mobileNumber });
  ...
  const handleUpdateJobDetails = async () => {
    if(!editJobData.farmerName || !editJobData.mobileNumber) return toast.error("Name and Mobile are required");
      ...
      <header className="bg-white/90 backdrop-blur-md sticky top-0 z-40 border-b border-gray-100 px-5 pt-10 pb-4 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            <button onClick={openEditJobModal} className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full transition-colors">
                <Pencil size={14} />
            </button>
          </div>
        </div>
      ...
      {showEditJobModal && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          ...
          <div className="bg-white w-full max-w-md rounded-3xl p-6 relative z-10 shadow-2xl animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-300">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1 mb-1 block">Name</label>
                <div className="relative">
                  <span className="absolute left-3 top-3 text-gray-400"><User size={16} /></span>
                  <input 
                    type="text" 
                    ...
                    onChange={(e) => setEditJobData({...editJobData, farmerName: e.target.value})} 
                  />
                </div>
              </div>

              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1 mb-1 block">Mobile Number</label>
                <div className="relative">
                  <span className="absolute left-3 top-3 text-gray-400"><Smartphone size={16} /></span>
                  <input 
                    type="tel" 
                    maxLength={10}
                    className="w-full pl-9 p-3 bg-gray-50 rounded-xl text-sm font-bold outline-none focus:bg-white focus:ring-2 focus:ring-gray-100" 
                    value={editJobData.mobileNumber} 
                    onChange={(e) => setEditJobData({...editJobData, mobileNumber: e.target.value})} 
                  />
                </div>
              </div>

              <button onClick={handleUpdateJobDetails} className="w-full bg-black text-white py-4 rounded-xl font-bold text-sm shadow-lg active:scale-95 transition">
                Update Details
              </button>
            </div>
          </div>
