...
export default function PublicPage() {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      try {
        ...
        toast.success(
          lang === "hi" ? "शेयर किया गया!" : "Shared successfully!",
        );
      } catch {}
    } else {
  ...
  const equipmentIcons = [
    <Settings key="1" size={20} />,
    <Tractor key="2" size={20} />,
    <Truck key="3" size={20} />,
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-32 md:pb-12">
      ...
      <div className="max-w-7xl mx-auto md:px-8">
        ...
        <div className="relative bg-emerald-900 text-white pt-6 pb-24 px-6 rounded-b-[2.5rem] md:rounded-[3rem] md:mt-4 shadow-2xl overflow-hidden">
          ...
          <div className="relative z-10 flex justify-between items-center mb-8 mt-2 md:mb-12">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full text-white text-xs font-bold border border-white/10 transition active:scale-95"
              >
                <Languages size={16} /> {lang === "hi" ? "English" : "हिंदी"}
              </button>
              <button
                onClick={handleShare}
                className="p-2 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full text-white transition active:scale-95 border border-white/10"
              >
                <Share2 size={20} />
              </button>
            </div>
          ...
          <div className="relative z-10 md:flex md:flex-col md:items-center md:text-center">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-800/50 border border-emerald-700 text-emerald-100 text-xs font-medium mb-4">
              <BadgeCheck size={14} className="text-yellow-400" /> {t.managedBy}
              : {t.ownerName}
            </div>
            ...
            <p className="text-emerald-100 text-sm sm:text-base mb-8 max-w-sm md:max-w-2xl leading-relaxed opacity-90">
              {t.heroDesc}
            </p>
            <div className="flex gap-3 w-full md:w-auto md:gap-6">
              <button
                onClick={handleCall}
                className="flex-1 md:flex-none md:w-48 bg-white text-emerald-900 py-3.5 px-4 rounded-xl font-bold text-sm md:text-base shadow-lg active:scale-95 transition flex items-center justify-center gap-2"
              >
                <Phone size={18} /> {t.callNow}
              </button>
              <button
                onClick={scrollToRates}
                className="flex-1 md:flex-none md:w-48 bg-emerald-800 text-white py-3.5 px-4 rounded-xl font-bold text-sm md:text-base border border-emerald-700 active:scale-95 transition flex items-center justify-center gap-2"
              >
                ...
                <div className="flex items-center justify-center gap-1">
                  ...
                  {stat.icon && (
                    <Star
                      size={14}
                      fill="#FACC15"
                      className="text-yellow-400 md:w-6 md:h-6"
                    />
                  )}
                </div>
                <p className="text-[10px] md:text-sm text-gray-500 font-bold uppercase tracking-wide mt-1">
                  {stat.label}
                </p>
              </div>
            ))}
        ...
        <div className="md:grid md:grid-cols-2 md:gap-8 md:mt-16">
          ...
          <div className="px-6 mt-14 md:mt-0">
            <div className="bg-gradient-to-br from-emerald-50 via-white to-white rounded-2xl p-6 border border-emerald-100 shadow-sm h-full">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h2 className="font-bold text-gray-900 text-lg md:text-xl leading-tight">
                    {t.ownerName}
                  </h2>
                  <p className="text-xs md:text-sm text-emerald-700 font-bold uppercase tracking-wider">
                    {t.shopName}
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {t.badges.map((badge, i) => (
                  <span
                    key={i}
                    className="text-[11px] md:text-xs bg-white px-3 py-1.5 rounded-full border border-emerald-100 text-emerald-700 font-semibold shadow-sm"
                  >
                    {badge}
                  </span>
                ))}
          ...
          <div className="px-6 mt-12 md:mt-0 flex flex-col justify-center">
            ...
            <div className="bg-orange-50 p-5 rounded-2xl border border-orange-100 flex items-center justify-between h-full">
              <div>
                <h3 className="font-bold text-orange-900 md:text-lg">
                  {t.season.special}
                </h3>
                <p className="text-xs md:text-sm text-orange-800/80 mt-1 max-w-[200px] md:max-w-xs">
                  {t.season.desc}
                </p>
              </div>
        ...
        <div className="px-6 mt-12 md:mt-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
            {t.equipments.list.map((item, idx) => (
              <div
                key={idx}
                className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between h-32 md:h-40 hover:shadow-md transition-shadow"
              >
        ...
        <div className="mt-14 bg-white py-10 md:rounded-3xl md:mx-6 md:shadow-sm md:border md:border-gray-50">
          <div className="px-6">
            <div className="flex items-center justify-between mb-8">
              ...
              <span className="text-xs md:text-sm text-emerald-600 font-bold bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">
                {t.gallery.recent}
              </span>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 auto-rows-[140px] md:auto-rows-[200px]">
              {/* Big Image (img1) */}
              <div className="row-span-2 col-span-1 md:col-span-1 rounded-2xl overflow-hidden shadow-md relative group border border-gray-100">
                <Image
                  src="/img1.jpeg"
                  alt="Work 1"
                  fill
              ...
              {/* small Image Top (img2) */}
              <div className="col-span-1 rounded-2xl overflow-hidden shadow-sm relative group border border-gray-100">
                <Image
                  src="/img2.jpeg"
                  alt="Work 2"
                  fill
              ...
              {/* small Image Middle (img3) */}
              <div className="col-span-1 rounded-2xl overflow-hidden shadow-sm relative group border border-gray-100 md:row-span-1">
                <Image
                  src="/img3.jpeg"
                  alt="Work 3"
                  fill
              ...
              {/* Wide Image Bottom (img4) */}
              <div className="col-span-2 rounded-2xl overflow-hidden shadow-md relative group border border-gray-100">
                <Image
                  src="/img4.jpeg"
                  alt="Work 4"
                  fill
        ...
        <div className="px-6 mt-14 md:mt-20">
          <h2 className="text-gray-900 font-bold text-xl md:text-2xl mb-6 text-center">
            {t.process.title}
          </h2>
          <div className="space-y-4 md:space-y-0 md:grid md:grid-cols-3 md:gap-6">
            {t.process.steps.map((step, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex md:flex-col md:text-center md:items-center items-center gap-4 bg-white p-4 md:p-6 rounded-2xl border border-gray-100 shadow-sm hover:border-emerald-200 transition-colors"
                >
                  <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-600 shrink-0 border border-emerald-100">
        ...
        <div className="md:grid md:grid-cols-2 md:gap-8 md:items-start">
          ...
          <div className="px-6 mt-14 md:mt-20">
            <div className="bg-red-50 p-5 md:p-8 rounded-2xl border border-red-100 text-center h-full flex flex-col justify-center items-center">
              <AlertTriangle
                className="text-red-500 mx-auto mb-3 md:w-10 md:h-10"
                size={28}
              />
              <h3 className="font-bold text-gray-900 md:text-xl">
                {t.emergency.title}
              </h3>
              <p className="text-xs md:text-sm text-gray-600 mt-1 mb-4 max-w-sm mx-auto">
                {t.emergency.desc}
              </p>
              <button
                onClick={handleCall}
                className="text-xs md:text-sm font-bold bg-white text-red-600 border border-red-200 px-4 py-2 md:px-6 md:py-3 rounded-full shadow-sm hover:bg-red-600 hover:text-white transition-colors"
              >
                {t.emergency.btn}
              </button>
            </div>
          ...
          <div className="px-6 mt-14 md:mt-20">
            <h2 className="text-gray-900 font-bold text-lg md:text-xl mb-5 flex items-center gap-2">
              ...
              {t.why.title} {lang === "en" && t.shopName}
            </h2>
            <div className="grid grid-cols-2 gap-3 md:gap-6">
              <div className="bg-white p-4 md:p-6 rounded-2xl border border-gray-100 shadow-sm">
                <Clock
                  className="text-emerald-600 mb-2 md:w-8 md:h-8"
                  size={24}
                />
        ...
        <div className="px-6 mt-14 md:mt-24">
          <div className="bg-emerald-50 p-6 md:p-10 rounded-[2rem] border border-emerald-100 text-center md:max-w-3xl md:mx-auto">
            <MapPin
              size={28}
              className="text-emerald-600 mx-auto mb-3 md:w-10 md:h-10"
            />
            <h2 className="text-gray-900 font-bold text-lg md:text-2xl mb-2">
              {t.areas.title}
            </h2>
            ...
            <div className="flex flex-wrap justify-center gap-2 md:gap-3">
              {areaList.map((area, i) => (
                <span
                  key={i}
                  className="text-[10px] md:text-sm font-bold bg-white text-gray-600 px-3 py-1.5 md:px-4 md:py-2 rounded-full border border-gray-200 shadow-sm"
                >
                  {area}
                </span>
              ))}
        ...
        <div className="px-6 mt-14 mb-8 md:mt-24">
          <h2 className="text-gray-900 font-bold text-xl md:text-2xl mb-5 text-center">
            {t.faq.title}
          </h2>
          <div className="space-y-3 md:grid md:grid-cols-2 md:gap-6 md:space-y-0">
            {t.faq.list.map((faq, index) => (
              <div
                key={index}
                className="bg-white border border-gray-200 rounded-2xl overflow-hidden transition-all h-fit"
              >
                <button
                  onClick={() =>
                    setOpenFaqIndex(openFaqIndex === index ? null : index)
                  }
                  className="w-full flex justify-between items-center p-4 md:p-5 text-left hover:bg-gray-50"
                >
                ...
                {openFaqIndex === index && (
                  <div className="px-4 pb-4 md:px-5 md:pb-5 bg-gray-50/50">
        ...
        <div className="px-6 mt-8 mb-24 md:mb-20 md:max-w-2xl md:mx-auto">
          ...
          <div className="bg-white p-5 md:p-8 rounded-2xl border border-gray-100 shadow-sm relative text-center">
            ...
            <p className="text-sm md:text-lg text-gray-600 italic mb-4">
              "{t.testimonials.text}"
            </p>
            <div className="flex items-center justify-center gap-3">
              <div className="w-8 h-8 md:w-10 md:h-10 bg-gray-200 rounded-full flex items-center justify-center text-xs md:text-sm font-bold text-gray-500">
                RP
              </div>
